    ...
    <div className="w-full h-20 bg-amazon_blue text-lightText sticky top-0 z-50"> 
        <div className="h-full w-full mx-auto inline-flex items-center justify-between gap-1 mdl:gap-3 px-4 ">
          ...
          <div className="text-xs">
            ...
            {/* searchbar */}
            <form onSubmit={handleSearch} className="flex-1 h-10 hidden md:inline-flex items-center justify-between relative">
                <input 
            ...
            {/* signin */}
            {
                userInfo ? (
                    ...
                    <div onClick={()=>signIn()} className="text-xs text-gray-100 flex flex-col justify-center px-2 border border-transparent hover:border-white cursor-pointer duration-300 h-[70%]">
                        <p>Hello, sign in</p>
                        <p className="text-white font-bold flex">Account & Lists{" "}<span><BiCaretDown/></span></p>
                    </div>
                )
            }
            ...
            {session && isAdmin(session.user?.email) && (
                <Link href="/admin" className="flex items-center px-2 border border-transparent hover:border-white cursor-pointer duration-300 h-[70%]">
                    <MdAdminPanelSettings className="text-xl text-white mr-1" />
